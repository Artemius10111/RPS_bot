...
async def handlers():
    ...
    async def say_hello(message: types.Message):
        ...
        async with db_session() as session:
            await session.merge(PlayerScore(user_id=message.from_user.id, score=0, user_name=message.from_user.username))
            await session.commit()


        await message.answer("Hello! Let's play rock, paper, scissors game!", reply_markup=hello_inline)
    ...
    async def stats(call: types.CallbackQuery):
        ...
        places = ['🥇', '🥈', '🥉']
        score_entries = [f"""{places[index] if len(places)>=index+1 else ""}{index+1}. @{item.user_name}: <b>{item.score}</b>""" for index, item in enumerate(players)]
        score_entries_text = "\n".join(score_entries)\
